...
Switches  (either upper or lower case):
		...
		Note:
		For seq files copied off a D64 disk, the text files are typlical terminate by a CR (0x0d),
		where as Windows text files are terminated by CRLF and Unix text files by LF

		Assembly source conventions used for native CBM assemblers were lines terminated
		by CR and files names using extension .asm,
		for this case the switches would be /U /K

		Also INCLUDE file (and .LIB and .FIL) should have the extension as part of the file name,
		such as:
		 .LIB SAMPLE.ASM
...
(As long as they are unique, most directives are also accepted abbreviated to 3 characters or more, e.g. `.NMLIST` = `.NMLIS` = `.NMLI` = `.NML`.)
	...
	Note:
	For compatibility with conventions of the MADS assembler, which allowed "." Dot directices to start in column one, the compiler has
	been modified to "space over" the directive is one of the valid directives begins in column one.
### Listing Control Directives

#### `.NAME`/ `.NAM` /`.TITLE` / `.TIL` *text*
...
`.IFDEF` evaluates true if the argument is both a legal symbol name, and has been previously defined in the source file. `.IFNDEF` evaluates true if the argument is either not a legal symbol name, or has not previously been defined.
	...
	Note:
	The MADS assmebler had an undocumented conditional in the form of:
	...
	The "greater than" symbol acts as the .ENDIF.
	
	A work around has been added that if the compiler processes a ">" in the first column, it will subsitite the string:
