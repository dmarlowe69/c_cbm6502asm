...
long evalsym() {
	...
	sym[i++] = toupper(*(lp - 1));
	while (isalpha(*lp) || isdigit(*lp) || *lp == '$' || *lp == '.' ||
	       *lp == '_') {
	...
	if (!ptr)
		insert_symbol(sym, 0L, 0, 0,
		              0); /* insert an undefined symbol */
	else
		addref(&ptr->xref,
		       0); /* add a reference to symbol is it exists */
	...
	if (pass == 2) display_error(error = 'U');
...
long evalocal() { /* evaluate a local symbol */
	...
	for (p = lp; isdigit(*p); p++)
		loc = loc * 10 + *p -
		      '0'; /* get the decimal # part of local symbol */
	if (*p == '$') {   /* yes it is a local symbol */
		lp = ++p;  /* point past the $ */
		if (loc < 1 ||
		    loc > 255) { /* it is a valid local symbol name */
			display_error(error = 'E');
			return (0L);
		}
		makelocal(loc, sym);
		ptr = find_symbol(
		        sym); /* get a pointer to the symbol table entry */
		if (!ptr)
			insert_symbol(sym, 0L, 0, 0,
			              1); /* insert an undefined symbol */
		if (ptr->section)         /* bad idea to use section names in
		                             expressions */
			display_error(error = 'S');
		if (ptr->multiple) /* use of multiply defined symbol */
			display_error(error = 'S');
...
int evaluate(char *str, long *value) {
	...
	while ((c = *lp)) {
		...
		if (c == '*') { /* multiply or location counter */
			/* if preceded by nothing or an operator then it is the
			 * location counter */
			if (!lastval)
				pushval(loc_cntr);
			else
				/* if preceded by a constant or symbol then it
		...
		if (c == '+') { /* unary or binary operator */
			/* if preceded by nothing or an operator then it is a
			 * unary +, ignore it */
			/* if preceded by a constant or symbol then it is a
			 * binary operator */
			if (lastval) eval(c);
			continue;
		}
	...
	while (opstk[osp] != '$') do_oper();
	for (i = osp; i < MAXESTK; i++)
		if (opstk[i] == '(')
			display_error(
			        error = 'B'); /* too many ( in expression */
	if (vsp != MAXESTK - 1)
		display_error(error = 'E'); /* stack was messed up */
	if (error) stack[vsp] = 0;
	*value = stack[vsp];
	return (expdef);
}
